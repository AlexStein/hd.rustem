...
function send_mail_to($type,$tid) {
    ...
    $MAIL_comment=lang('MAIL_comment');
    $MAIL_author=lang('MAIL_author');
    $MAIL_updated=lang('MAIL_updated');
...
EOBODY;
    ...
    if ($type == "comment_all") {
        ...
        $stmt = $dbConnection->prepare('SELECT user_id, comment_text, dt from comments where t_id=:tid order by dt DESC LIMIT 1');
        $stmt->execute(array(':tid' => $tid));
        $rews = $stmt->fetch(PDO::FETCH_ASSOC);
        $user_id = $rews['user_id'];
        $comment_text = $rews['comment_text'];
        $cuin=name_of_user_ret($user_id);
            ...
            foreach ($u as $val) {
                if ($val== $unit_id) {
                    if (!is_null($qrow['email'])) {
                        ...
                        $message =<<<EOBODY
<div style="background: #ffffff; border: 1px solid gray; border-radius: 6px; font-family: Arial,Helvetica,sans-serif; font-size: 12px; margin: 9px 17px 13px 17px; padding: 11px;">
<p style="font-family: Arial, Helvetica, sans-serif; font-size:18px; text-align:center;">{$MAIL_updated}!</p>
<table width="100%" cellpadding="3" cellspacing="0">
  <tbody>
    <tr id="tr_">
      <td width="15%" style="border: 1px solid #ddd;font-family: Arial, Helvetica, sans-serif;
...
<table width="100%" cellspacing="0" cellpadding="3" style="">
  ...
  <tr>
    <td colspan="2"  style="border: 1px solid #ddd;font-family: Arial, Helvetica, sans-serif;
	font-size: 12px; background-color: #f5f5f5;"><center>
      <strong>{$MAIL_comment}</strong>
    </center></td>
  </tr>
  <tr>
    <td style="border: 1px solid #ddd;font-family: Arial, Helvetica, sans-serif;
	font-size: 12px;">{$MAIL_author}:</td>
    <td style="border: 1px solid #ddd;font-family: Arial, Helvetica, sans-serif;
	font-size: 12px;">{$cuin}</td>
  </tr>
  <tr>
    <td style="border: 1px solid #ddd;font-family: Arial, Helvetica, sans-serif;
	font-size: 12px;">{$MAIL_text}:</td>
